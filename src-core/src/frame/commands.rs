...
#[tauri::command]
pub fn frame_supported_models() -> Vec<SupportedModel> {
    SUPPORTED_MODELS.to_vec()
}

#[tauri::command]
pub async fn frame_discover() -> Vec<Candidate> {
    discovery::discover(Duration::from_secs(3)).await
}

/// `None` when no devkit service answers at the address.
#[tauri::command]
pub async fn frame_login_name(address: String) -> Option<String> {
...
#[tauri::command]
pub async fn frame_create_credentials() -> Result<Credentials, String> {
...
#[tauri::command]
pub async fn frame_register(address: String, public_key: String) -> RegisterOutcome {
...
#[tauri::command]
pub async fn frame_probe(access: Access) -> ProbeOutcome {
...
#[tauri::command]
pub async fn frame_setup(request: SetupRequest) -> SetupResult {
    ...
    setup::setup(request, move |stage| {
        ...
        tokio::spawn(send_event("FRAME_SETUP_STAGE", event));
    })
    .await
}

#[tauri::command]
pub async fn frame_cleanup(request: CleanupRequest) -> CleanupOutcome {
    setup::cleanup(request).await
}

#[tauri::command]
pub async fn frame_set_pairings(pairings: Vec<Pairing>) {
    connection::set_pairings(pairings).await
}

#[tauri::command]
pub async fn frame_connection_states() -> Vec<State> {
