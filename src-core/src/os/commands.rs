...
pub async fn run_cmd_commands(commands: String) {
    ...
    match File::create(&batch_path).await {
        Ok(mut file) => {
            ...
            debug!("[Core] Successfully wrote {} bytes to batch file", commands.len());
    ...
    debug!("[Core] Verified batch file exists: {}", batch_path.display());
    ...
    debug!(
        "[Core] Attempting to spawn detached cmd.exe process with batch file: {}",
        batch_path.to_str().unwrap()
    );
    
    match cmd_builder.spawn() {
        Ok(child) => {
            debug!("[Core] Successfully spawned detached cmd.exe process with PID: {:?}", child.id());
        }
        Err(e) => {
