...
mod tests {
    ...
    // An executable that cannot be found fails the spawn without starting a process, which is
    // the launch failure these tests need and costs nothing to produce.
    fn manager(auto_restart: bool, on_stop_tx: mpsc::Sender<()>) -> SidecarManager {
        SidecarManager::new(
            "TEST".to_string(),
            "resources/no-such-directory/".to_string(),
            "no-such-sidecar.exe".to_string(),
            on_stop_tx,
            auto_restart,
            vec![],
        )
    ...
    // The manager launches nothing until the core's own gRPC port is known.
    async fn set_core_grpc_port() {
        *crate::grpc::SERVER_PORT.lock().await = Some(1);
    }

    async fn expect_stop(rx: &mut mpsc::Receiver<()>, within: Duration, what: &str) {
        timeout(within, rx.recv())
            .await
            .unwrap_or_else(|_| panic!("no stop signal: {what}"));
    }

    #[tokio::test]
    async fn a_launch_that_never_happens_reports_and_retries() {
        set_core_grpc_port().await;
        let (tx, mut rx) = mpsc::channel(10);
        let mut manager = manager(true, tx);
        manager.start().await;
        expect_stop(&mut rx, Duration::from_secs(1), "the failed launch").await;
        expect_stop(&mut rx, Duration::from_secs(2), "the first retry").await;
        expect_stop(&mut rx, Duration::from_secs(3), "the second retry").await;
    }

    #[tokio::test]
    async fn an_explicit_restart_does_not_wait_out_the_backoff() {
        set_core_grpc_port().await;
        let (tx, mut rx) = mpsc::channel(10);
        let mut manager = manager(true, tx);
        manager.start().await;
        // walk the ladder up to its 5 second step
        for _ in 0..4 {
            expect_stop(&mut rx, Duration::from_secs(5), "the warm up").await;
        }
        let requested_at = Instant::now();
        manager.start_or_restart().await;
        expect_stop(&mut rx, Duration::from_secs(3), "the explicit restart").await;
        assert!(
            requested_at.elapsed() < Duration::from_secs(2),
            "the restart waited out the backoff: {:?}",
            requested_at.elapsed()
        );
    }

    #[tokio::test]
    async fn a_manager_that_does_not_auto_restart_gives_up() {
        set_core_grpc_port().await;
        let (tx, mut rx) = mpsc::channel(10);
        let mut manager = manager(false, tx);
        manager.start().await;
        expect_stop(&mut rx, Duration::from_secs(1), "the failed launch").await;
        assert!(
    ...
    #[tokio::test]
    async fn a_restart_during_a_launch_does_not_deadlock() {
        set_core_grpc_port().await;
        let (tx, mut rx) = mpsc::channel(50);
        let mut launching = manager(true, tx);
        let mut restarting = launching.clone();
        let launch = tokio::spawn(async move { launching.start().await });
        let restart = tokio::spawn(async move { restarting.start_or_restart().await });
        timeout(Duration::from_secs(5), launch)
            .await
            .expect("the launch deadlocked")
            .unwrap();
        timeout(Duration::from_secs(5), restart)
            .await
            .expect("the restart deadlocked")
            .unwrap();
        expect_stop(&mut rx, Duration::from_secs(2), "the failed launch").await;
    }
}
