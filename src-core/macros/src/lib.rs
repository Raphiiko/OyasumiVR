...
pub fn command_profiling_async(_args: TokenStream, input: TokenStream) -> TokenStream {
    ...
    let new_body = quote! {
        {
            let __invocation_id = #start_fn_path(#fn_name_str);
...
#[proc_macro_attribute]
pub fn command_profiling_sync(_args: TokenStream, input: TokenStream) -> TokenStream {
    let input_fn = parse_macro_input!(input as ItemFn);
    ...
    // Parse the full paths as expressions
    let start_fn_path: syn::ExprPath = parse_str("crate::utils::profiling::profile_command_start")
        .expect("Failed to parse start function path");
    let end_fn_path: syn::ExprPath = parse_str("crate::utils::profiling::profile_command_finish")
        .expect("Failed to parse end function path");
