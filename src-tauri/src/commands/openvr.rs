...
#[tauri::command]
pub fn openvr_set_supersample_scale(supersample_scale: Option<f32>) -> Result<(), String> {
    let manager_guard = OPENVR_MANAGER.lock().unwrap();
    let manager = manager_guard.as_ref().unwrap();
    manager.set_supersample_scale(supersample_scale)
}

#[tauri::command]
pub fn openvr_get_supersample_scale() -> Result<Option<f32>, String> {
    let manager_guard = OPENVR_MANAGER.lock().unwrap();
    let manager = manager_guard.as_ref().unwrap();
    manager.get_supersample_scale()
}
