...
const FRAME_STATUS_ROWS: Partial<Record<SteamFrameConnectionStatus, FrameRow>> = {
  identityChanged: {
    pill: { key: 'notRecognized', icon: 'error', tone: 'bad', detail: 'notRecognized' },
    action: 'pairAgain',
  },
  hostKeyChanged: {
    pill: { key: 'notRecognized', icon: 'error', tone: 'bad', detail: 'notRecognized' },
...
export class DeviceManagerDevicesTabComponent implements OnInit, AfterViewInit {
  ...
  frameRow(device: DMKnownDevice): FrameRow | null {
    ...
    if (reinstall === 'failed' && status === 'helperMissing') {
      return {
        pill: { key: 'reinstallFailed', icon: 'error', tone: 'bad', detail: 'reinstallFailed' },
        action: 'reinstall',
      };
    }
  ...
  private frameMaintenanceRow(state?: SteamFrameConnectionState): FrameRow | null {
    ...
    switch (maintenance?.kind) {
      ...
      case 'busy':
        return {
          pill: { key: 'helperBusy', icon: 'hourglass_empty', tone: 'warn', detail: 'helperBusy' },
          action: 'retryUpdate',
        };
      default:
        return null;
  ...
  onFramePill(device: DMKnownDevice, pill: FramePill) {
    ...
    this.modalService
      .addModal<ConfirmModalInputModel, ConfirmModalOutputModel>(ConfirmModalComponent, {
        ...
        message: `steamFrame.statusDetail.${pill.detail}.${pill.detailBody ?? 'body'}`,
