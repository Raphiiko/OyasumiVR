...
import { FramePairing } from '../../../models/frame';
...
import { FramePairingMessageMonitor } from './frame-pairing-message-monitor';

vi.mock('../../device-manager.service', () => ({ DeviceManagerService: class {} }));
vi.mock('../../frame-pairing.service', () => ({ FramePairingService: class {} }));
...
function start() {
  ...
  const pairings$ = new BehaviorSubject<FramePairing[]>([]);
  ...
  const injector = Injector.create({
    providers: [
      ...
      { provide: FramePairingService, useValue: framePairing },
    ],
  });
  const monitor = runInInjectionContext(
    injector,
    () => new FramePairingMessageMonitor(messageCenter)
  );
  monitor.init();
...
describe('Steam Frame pairing invitation', () => {
  it('invites a connected, unpaired Frame under an id of its own, which hiding persists', () => {
    ...
    expect(message).toMatchObject({ title: 'frame.invitation.title', hideable: true });
  ...
  it('withdraws the invitation once paired or disconnected', () => {
    const h = start();
    h.pairings$.next([{ deviceId: h.frame.id, complete: true } as FramePairing]);
