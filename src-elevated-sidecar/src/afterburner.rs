...
fn verify_afterburner_signature(exe_path: String) -> Result<String, String> {
    let signature = match CodeSigned::new(exe_path.clone()) {
        ...
        Err(_) => {
            warn!("[Afterburner] Executable at the path provided ({}) could not be verified with a signature.", exe_path.clone());
            return Err(String::from("EXE_UNVERIFIABLE"));
        }
    };
    if !signature.is_signed() {
        warn!("[Afterburner] Executable at the path provided ({}) does not have a signature and could not be verified.", exe_path.clone());
        return Err(String::from("EXE_NOT_SIGNED"));
    }
    if !signature.is_embedded() {
        warn!("[Afterburner] Signature found for the executable at the path provided ({}) is not permitted: Non-embedded signature.", exe_path.clone());
    ...
    let issuer_name = match signature.issuer_name {
        ...
        None => {
            warn!("[Afterburner] Signature found for the executable at the path provided ({}) is not permitted: No issuer found.", exe_path.clone());
    ...
    let subject_name = match signature.subject_name {
        ...
        None => {
            warn!("[Afterburner] Signature found for the executable at the path provided ({}) is not permitted: No subject found.", exe_path.clone());
    ...
    if !allowed_issuers_and_subjects.contains(&issuer_and_subject) {
        warn!("[Afterburner] Signature found for the executable at the path provided ({}) is not permitted (issuer_name:[{}], subject_name:[{}])", exe_path.clone(), issuer_name, subject_name);
        return Err(String::from("EXE_SIGNATURE_DISALLOWED_NO_MATCH"));
    }
    info!("[Afterburner] Successfully verified signature for the executable at the path provided({}).", exe_path.clone());
