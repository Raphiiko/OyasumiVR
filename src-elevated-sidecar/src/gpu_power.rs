use crate::Models::{NvmlDevice, NvmlSetPowerManagementLimitError, NvmlStatus};
use crate::{adlx_backend, nvml_backend};
use log::info;
use std::sync::Mutex;

lazy_static! {
    static ref GPU_POWER_STATUS: Mutex<NvmlStatus> = Mutex::new(NvmlStatus::Initializing);
}

pub fn init() -> bool {
    info!("[GPU] Initializing power-limit backends (NVML + ADLX)");

    let mut fallback_status = NvmlStatus::NvmlUnknownError;
    let nvml_initialized = match nvml_backend::init() {
        Ok(()) => true,
        Err(status) => {
            fallback_status = status;
            false
        }
    };

    let adlx_initialized = adlx_backend::init();
    let initialized = nvml_initialized || adlx_initialized;

    *GPU_POWER_STATUS.lock().unwrap() = if initialized {
        NvmlStatus::InitComplete
    } else {
        fallback_status
    };

    initialized
}

pub async fn status() -> NvmlStatus {
    *GPU_POWER_STATUS.lock().unwrap()
}

pub fn get_devices() -> Vec<NvmlDevice> {
    let mut devices = nvml_backend::get_devices();
    devices.extend(adlx_backend::get_devices());
    devices
}

pub async fn set_power_management_limit(
    uuid: String,
    limit: u32,
) -> Result<bool, NvmlSetPowerManagementLimitError> {
    if adlx_backend::is_adlx_uuid(&uuid) {
        return adlx_backend::set_power_management_limit(uuid, limit);
    }

    nvml_backend::set_power_management_limit(uuid, limit)
}
